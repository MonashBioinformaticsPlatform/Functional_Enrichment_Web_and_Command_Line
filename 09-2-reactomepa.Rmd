...
```{r gprofiler2-echo-setup, include=FALSE}
...
```{r Load R packages, include=TRUE, eval=TRUE}
library(ReactomePA)
...
```{r working directory, include=TRUE, eval=TRUE}
...
```{r load input data, include=TRUE, eval=TRUE}
...
```{r get Entrez IDs, include=TRUE, eval=TRUE}
...
```{r, Get rid of duplicates, include=TRUE, eval=TRUE}
...
```{r, get DEGs, include=TRUE, eval=TRUE}
...
```{r Run Reactome ORA, include=TRUE, eval=TRUE, cache=TRUE}
...
```{r make pretty table of top entries, include=TRUE, eval=TRUE}
...
```{r plot ORA results, include=TRUE, eval=TRUE}
ggplot(data=RPA_df,
...
```{r, calculate pi-scores, include=TRUE, eval=TRUE}
...
```{r make gsePA input, include=TRUE, eval=TRUE}
pi_vals <- pezzini$pi  
...
```{r run gsePA, include=TRUE, eval=TRUE, cache=TRUE}
...
```{r display gsePA results as table, include=TRUE, eval=TRUE}
...
```{r barcode plot top PAs, include=TRUE, eval=TRUE}
...
```{r barcode plot upreg PAs, include=TRUE, eval=TRUE}
...
```{r display pathways, include=TRUE, eval=TRUE}
...
```{r show export table, include=TRUE, eval=TRUE}
...
```{r restore working directory, include=FALSE, eval=TRUE}
