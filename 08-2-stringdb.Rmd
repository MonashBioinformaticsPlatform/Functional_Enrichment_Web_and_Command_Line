...
[STRING](https://string-db.org) is a database of known and predicted protein-protein interactions, including both direct (physical) and indirect (functional) associations, drawn from experimental repositories, computational prediction methods, and text-mined publications. Each interaction is given a combined confidence score integrating all of the underlying evidence. STRING v12.0 covers over 59 million proteins from more than 12,500 organisms.
...
This chapter is based on the [STRINGdb package vignette](https://bioconductor.org/packages/release/bioc/vignettes/STRINGdb/inst/doc/STRINGdb.pdf), reworked to use our own [Pezzini et al. 2017](https://link.springer.com/article/10.1007/s10571-016-0403-y) SH-SY5Y neuronal differentiation dataset in place of the vignette's example data, so that you can compare the R output directly against what you saw on the STRING website earlier.
...
## 1. Initialise the STRINGdb object
...
## 3. Load the differential expression data
...
```{r stringdb-load-de}
de_full <- read_csv("data/Pezzini2016_SHSY5Ycelldiff_DE_table.csv", show_col_types = FALSE)
head(de_full)
```
...
de_198 <- de_full %>%
  filter(FDR < 0.0001 & abs(logFC) >= 1) %>%
  transmute(gene = Gene.Name, pvalue = FDR, logFC = logFC)

cat("Number of genes passing FDR and fold change filter:", nrow(de_198), "\n")
head(de_198)
```

## 4. Map gene identifiers to STRING IDs
...
de_198_mapped <- string_db$map(de_198, "gene", removeUnmappedRows = TRUE)
head(de_198_mapped)
```
...
## 5. Visualise the interaction network
...
## 6. Payload mechanism: colour by direction of change
...
```{r stringdb-add-diff-exp-colour}
de_198_coloured <- string_db$add_diff_exp_color(de_198_mapped, logFcColStr = "logFC")
```

```{r stringdb-post-payload}
payload_id <- string_db$post_payload(de_198_coloured$STRING_id,
                                      colors = de_198_coloured$color)
```

```{r stringdb-plot-halo-network, fig.width=8, fig.height=8}
...
## 8. Setting a custom statistical background
...
background_v <- string_db$map(
  transmute(de_full, gene = Gene.Name),
  "gene", removeUnmappedRows = TRUE
)$STRING_id

string_db$set_background(background_v)
```
...
## 9. Retrieve functional annotations
...
## 11. Querying specific proteins of interest
...
string_db$get_interaction_partners(c(crabp2, sema3a), limit = 5)[
  , c("from", "to", "combined_score", "experimental", "from_name", "to_name")]
```
...
```{r stringdb-paralogs}
string_db$get_paralogs(crabp2)
```

STRING also stores cross-species best hits (by bitscore). `get_homologs_besthits` retrieves these:

```{r stringdb-homologs-besthits}
crabp2_besthits <- string_db$get_homologs_besthits(crabp2)
crabp2_besthits_100 <- subset(crabp2_besthits, bitscore > 100)
nrow(crabp2_besthits_100)
head(crabp2_besthits_100, n = 10)
```

... or restrict to a specific target species, e.g. mouse (taxonomy ID `10090`):

```{r stringdb-homologs-besthits-mouse}
string_db$get_homologs_besthits(c(crabp2, sema3a), target_species_id = 10090)
```

## 12. Additional protein information
