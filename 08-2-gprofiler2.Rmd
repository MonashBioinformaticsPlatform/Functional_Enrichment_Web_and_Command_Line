...
[gprofiler2](https://cran.r-project.org/web/packages/gprofiler2/index.html) is the R interface to the `g:Profiler` web-based toolset that you used earlier in the book.
...
Since we are doing ORA, we will need a filtered gene list, and a background gene list. We will continue with the RNAseq dataset from [Pezzini et al 2017](https://link.springer.com/article/10.1007/s10571-016-0403-y) introduced earlier in the book.
...
```{r working directory, eval=TRUE, include = FALSE}
...
gprofiler2_book_root_dir <- getwd()
...
filename <- "Pezzini_DEGs.txt"
write.table(degs, filename, row.names = FALSE, col.names = FALSE, quote = FALSE)
...
Recall from the webinar and earlier in the book that an experimental background gene list is crucial to avoiding false positives and minimising tissue bias with ORA.
...
filename <- "Pezzini_background_genes.txt"
write.table(background, filename, row.names = FALSE, col.names = FALSE, quote = FALSE)

cat("Table saved to", filename, "\n")
```

## Run ORA with `gost` function
...
An error-free `gost` run should produce no console output.  As the code is running, there will be a green bar to the left of the code chunk.
...
Same as the web tool, we have enrichment results for GO (BP, CC, MF), HP (human phenotype), HPA (human protein atlas), KEGG, MiRNA, Reactome, Transcription Factors, and WikiPathways. 
...
```{r write ora to csv}
# print to CSV
filename <- "gprofiler_ORA_results.csv"
write.csv(ora_reordered, filename, row.names = FALSE)
...
We'll reuse the same source-database colour palette across all of the `gostplot` calls below, so let's define it once:
...
# specify term IDs for terms of interest: 'Collagen degradation' and 'Collagen formation'
...
filename <- "gprofiler_GO_BP_dotplot.pdf"
pdf(filename, width = 8.27, height = 11.69) # A4 portrait size in inches
...
# Save the DEG gene list to disk:
filename <- "up_DEGs.txt"
write.table(up_degs, filename, row.names = FALSE, col.names = FALSE, quote = FALSE)
cat("Up-regulated DEGs saved to", filename, "\n")
...
# Save the DEG gene list to disk:
filename <- "down_DEGs.txt"
write.table(down_degs, filename, row.names = FALSE, col.names = FALSE, quote = FALSE)

cat("Down-regulated DEGs saved to", filename, "\n")
```
...
# Print TSV file
filename <- "gprofiler_ORA_ora_multiquery.tsv"
write.table(ora_multi$result, filename, row.names = F, quote = F, sep = "\t")

cat("ORA multi-query results written to", filename, "\n")
```




## Compare gprofiler2 R results to g:Profiler web results

Earlier in the book, you ran ORA with g:Profiler web tool and saved the results to a CSV. Let's compare the results to those we have generated in R. Do we expect the results to be identical or differ slightly?
...
web <- read.csv("gProfiler_hsapiens_2026-08-09_05-01-55__intersections.csv")
head(web)
```
...
```{r compare web vs r term counts}
...
Let's check: earlier in the book when you ran ORA on the web, hopefully you saved your 'query parameters' as well as your results.
...
knitr::opts_knit$set(root.dir = gprofiler2_book_root_dir)
```
