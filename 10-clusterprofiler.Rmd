...
```{r clusterprofiler-echo-setup, include=FALSE}
...
| Supported namespaces | Differs depending on species                    | `kegg` (compatible with Entrez), `ncbi-geneid`, `ncbi-proteinid`, or `uniprot` | `kegg` (compatible with Entrez), `ncbi-geneid`, `ncbi-proteinid`, or `uniprot` | `hgnc` or `uniprot`              | Entrez       | Those supported by DAVID |
...
4. Use `bitr` to convert gene IDs from ENSEMBL to ENTREZ for compatibility with `gseKEGG`
...
```{r Load R packages clusterprofiler, include=TRUE, eval=TRUE}
...
```{r check notebook workdir, include = FALSE, eval=TRUE}
...
clusterprofiler_book_root_dir <- getwd()
knitr::opts_knit$set(root.dir = "day2_inputData")
getwd()
```

## Load input data and extract ranked gene list

We will once again load the Pezzini RNAseq dataset.

```{r load input data clusterprofiler}
...
## Check `gseKEGG` function arguments and requirements
...
```{r check org db keytypes for human}
...
As in the previous `gprofiler2` chapter, we'll spell out every parameter explicitly below, even where the default would do - this keeps your parameter choices visible and documented, and protects you if a package's defaults ever change between versions.
...
Note also that `enrichplot`'s `treeplot` API has itself changed between versions - the `cluster.params` argument used above has since been replaced by a simpler `nCluster` argument:
...
enrichplot::treeplot(gsea_kegg, showCategory = 15, color = "p.adjust", nCluster = 5)
```
...
```{r save clusterprofiler heatplot to file}
...
The term 'hsa04062' (Chemokine signaling pathway) has a negative NES, a highly significant adjusted P value of 1.11E-06 and a set size of 129.
...
```{r info clusterprofiler}
sessionInfo()
```

```{r restore working directory clusterprofiler, include=FALSE, eval=TRUE}
...
knitr::opts_knit$set(root.dir = clusterprofiler_book_root_dir)
```
...
- We have extracted a ranked gene list for GSEA and converted the gene IDs for compatibility with `gseKEGG`
